...
export function initMapFromKeys({ keys, initValue }) {
  const thisMap = new Map();
  keys.forEach((key) => {
    thisMap.set(key, initValue);
  });
  return thisMap;
}

export function initObjFromKeys({ keys, initValue }) {
  const obj = {};
  keys.forEach((key) => {
    obj[key] = initValue;
  });
...
export function calcLoading(statuses) {
  let isLoading = false;
...
export function inputValuesInvalid(input) {
  const check = Object.keys(input).map((inp) => {
    if (input[inp].length <= 0 || input[inp] === false || input[inp] === '')
      return true;
    return false;
  });
