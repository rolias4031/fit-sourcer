...
export const useEditGarment = () => {
  const { mutate: editGarment, status: editStatus } =
    useMutation(mutateWithBody);
  ...
  return { editGarmentHandler, editStatus };
};
...
export const useUploadGarmentImage = () => {
  ...
  const { mutate: uploadGarmentImage, status } = useMutation(async (image) => {
...
export const useDeleteGarmentMutation = () => {
  const client = useQueryClient();
  const router = useRouter();

  const { mutate, status: deleteStatus } = useMutation(mutateWithBody, {
    onSuccess: () => {
  ...
  return { deleteGarmentHandler, deleteStatus };
};
